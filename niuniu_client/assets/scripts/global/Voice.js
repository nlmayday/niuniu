cc.Class({
    ...
    onLoad: function () {
        this._voice = cc.find("Canvas/table/voice");
        this._voice.active = false;
        
        this._voice_failed = cc.find("Canvas/table/voice/voice_failed");
        this._voice_failed.active = false;
        
        this._timeBar = cc.find("Canvas/table/voice/time");
        this._timeBar.scaleX = 0.0;
        
        this._volume = cc.find("Canvas/table/voice/volume");
        for(var i = 1; i < this._volume.children.length; ++i){
            this._volume.children[i].active = false;
        }
        
        var btnVoice = cc.find("Canvas/table/voice/voice_failed/btn_ok");
        if(btnVoice){
            cc.vv.utils.addClickEvent(btnVoice,this.node,"Voice","onBtnOKClicked");
        }
        
        var self = this;
        var btnVoice = cc.find("Canvas/table/btn_voice");
        if(btnVoice){
    ...
    onVoiceOK:function(){
        if(this._lastTouchTime != null){
            ...
            cc.vv.netMgr.doSend2(cc.vv.netMgr.req.voice,{msg:msg,time:time});
        }
        this._voice.active = false;
    },
